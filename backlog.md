...
exists; items marked _(no issue)_ are proposals that came out of reading the code, not the tracker.
...
- [ ] The key reveal element is excluded from session recordings via the existing mask selector.
- [ ] Any network response body containing the plaintext key is excluded from capture.
- [ ] A test or lint guard prevents the reveal from being re-rendered unmasked.
- [ ] Verified against a deployment with `POSTHOG_PUBLIC_KEY` / `POSTHOG_HOST` set.
- [ ] Self-hosted deployments without PostHog remain unaffected.
...
### 5. Add a private vulnerability reporting channel — _(no issue; raised inside #233)_
...
### 10. Extract competitor positions from the SERP response already being paid for — _(no issue)_
...
### 21. Generate `llms.txt`, not just check for it — _(no issue; extends #99)_
...
### 22. Audit diffs and scheduled audits — _(no issue)_
...
### 23. Internal linking recommendations from the crawl graph — _(no issue)_
...
high-level" note never fires. Note the roadmap lists JS rendering as not planned for _site audit_;
...
### 32. Page-path and metric filters on GSC Insights — [#68](https://github.com/every-app/open-seo/issues/68) _(good first issue, status:ready)_
...
### 33. Paginated domain keyword discovery on the rank tracking page — [#50](https://github.com/every-app/open-seo/issues/50) _(status:ready)_
...
### 34. Per-keyword rank tracking intervals — [#37](https://github.com/every-app/open-seo/issues/37) _(status:spec-first)_
...
### 36. Multi-select keyword research types and autocomplete ideas — [#32](https://github.com/every-app/open-seo/issues/32) _(status:spec-first)_
...
### 37. Include rank check date in Rank Tracking exports — [#70](https://github.com/every-app/open-seo/issues/70) _(good first issue, status:ready)_
...
### 38. Cited pages tab in Prompt Explorer — [#66](https://github.com/every-app/open-seo/issues/66) _(status:ready)_
